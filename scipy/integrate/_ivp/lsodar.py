...
class LSODAR(OdeSolver):
    ...
    def __init__(self, fun, t0, y0, t_bound, first_step=None, min_step=0.0,
        ...
        if first_step is None:
            first_step = 0  # LSODAR value for automatic selection.
        elif first_step <= 0:
            raise ValueError("`first_step` must be positive or None.")

        if max_step == np.inf:
            max_step = 0  # LSODAR value for infinity.
        elif max_step <= 0:
        ...
        self._lsodar_solver = solver

    def _step_impl(self):
        solver = self._lsodar_solver
        integrator = solver._integrator

        # From lsodar.step and lsodar.integrate itask=5 means take a single
        ...
        if solver.successful():
            self.t = solver.t
            self.y = solver._y
            # From LSODAR Fortran source njev is equal to nlu.
            self.njev = integrator.iwork[12]
            self.nlu = integrator.iwork[12]
            return True, None
        else:
            return False, 'Unexpected istate in LSODAR.'

    def _dense_output_impl(self):
        iwork = self._lsodar_solver._integrator.iwork
        rwork = self._lsodar_solver._integrator.rwork
        ...
        return LsodarDenseOutput(self.t_old, self.t, h, order, yh)


class LsodarDenseOutput(DenseOutput):
    def __init__(self, t_old, t, h, order, yh):
        super(LsodarDenseOutput, self).__init__(t_old, t)
        self.h = h
        self.yh = yh
